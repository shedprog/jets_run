...
//  Int_t q2end   = 350;
  ...
  TH2D* h2ElDecorrPhi[5];
  TH2D* h2Q2DecorrPhi[5];
...
public:
  ...
  Selector(TTree *tree, Bool_t isdata) : JetOrangeSelect(tree,isdata) {};
  ...
  void InitHists(){
	  ...
	  for(Int_t ijet =0; ijet < 5; ijet++){ 
    	...
    	h2ElDecorrPhi[ijet] = new TH2D(Form("h2ElDecorrPhi_%d",ijet),
				   Form("Energy of the Lepton vs angle, %d jets",ijet+1), 60,TMath::Pi()/2,TMath::Pi(),150,10,25);
    	h2Q2DecorrPhi[ijet] = new TH2D(Form("h2Q2DecorrPhi_%d",ijet),
				   Form("Q^{2} vs angle, %d jets",ijet+1), 60,TMath::Pi()/2,TMath::Pi(),q2end-q2start, q2start, q2end);
	  }

	  h2JetElecPhi  = new TH2D("h2JetElecPhi" ,"Jet vs electron #phi angle", 60,-TMath::Pi(),TMath::Pi(), 60,0,2*TMath::Pi());
	  ////END Define Histograms <--2--
  };

  void FillHists_reco(bool isdata, bool isCalibr){
    ...
    Float_t JetPt = sqrt(Kt_etjet_b[0]*Kt_etjet_b[0]-Kt_masjet_b[0]*Kt_masjet_b[0]) ;
  	...
  	/////////////////////Fill histograms --3.2-->
    //if(reweight) Weight = CalcWeight(Siph[0],Kt_phijet_b[0]);
    ...
    for(Int_t ijet =0; ijet < 5; ijet++){ 
      if(Kt_njet_b > ijet) {
      	...
      	h2ElDecorrPhi[ijet]->Fill(DecorrPhi, Siecorr[0][2],Weight);
        // std::cout<<ijet<<" "<<DecorrPhi<<" "<<Siq2el[0]<<"\n";
      	h2Q2DecorrPhi[ijet]->Fill(DecorrPhi, Siq2el[0],Weight);
      }
    }
    
    h2JetElecPhi->Fill(Siph[0],Kt_phijet_b[0],Weight);

    ///////////////MC information --3.2.1-->
  };

  void FillHists_true(bool isdata, bool isCalibr){
    ...
    jet_final.SetPxPyPzE( Pxhmjet[highEt_jet], Pyhmjet[highEt_jet], 
  	 ...
  	 /////////////////////Fill histograms --3.2-->
    //if(!isdata) Weight = CalcWeight(Siph[0],Kt_phijet_b[0]);
    //cout << "Weight   : " << Weight << endl; 
    //if(Weight ==0) continue; 
    //------------Event
    hVertexZ->Fill(Mc_vtx[2]                         ,Weight); 
    h2Q2x   ->Fill(Mc_x,Mc_q2 ,Weight);
    hQ2     ->Fill(Mc_q2                     ,Weight);
    hx      ->Fill(Mc_x                      ,Weight);
    hJetMult->Fill(Nhmjets                     ,Weight);
    //    hEmpz   ->Fill(Empz                                     ,Weight);
    //hEmpz   ->Fill(Cal_empz                      ,Weight);
    //hGamma  ->Fill(TMath::Cos(Cc_gamma)          ,Weight);
    //hPtEt   ->Fill(Cal_pt/sqrt(Cal_et),Weight);
    ...
    for(Int_t ijet =0; ijet < 5; ijet++){ 
      if(Nhmjets > ijet) {
        ...
        h2Q2DecorrPhi[ijet]->Fill(DecorrPhi, Mc_q2,Weight);
      }
    }
    // std::cout<<lepton_final.Phi()<<" "<<jet_final.Phi()<<"\n";
